...
describe('relationshipMoments', () => {
  it('keeps only weekday childcare overlap at home before moving in', () => {
    ...
    expect(moments).toEqual([{ date: '2026-10-30', station: 'ALA', kind: 'private', interval: { start: 8 * 60, end: 17 * 60 }, minutes: 9 * 60 }]);
  });
  ...
  it('keeps the non-childcare part as family time after moving in', () => {
    ...
    expect(moments).toEqual([
      { date: '2026-11-02', station: 'ALA', kind: 'family', interval: { start: 7 * 60, end: 8 * 60 }, minutes: 60 },
      { date: '2026-11-02', station: 'ALA', kind: 'private', interval: { start: 8 * 60, end: 17 * 60 }, minutes: 9 * 60 },
      { date: '2026-11-02', station: 'ALA', kind: 'family', interval: { start: 17 * 60, end: 19 * 60 }, minutes: 120 },
    ]);
  });
